...
public class CapController {
    ...
    public void saveCap(@RequestBody java.util.Map<String, Object> requestData){
        // Cap을 Product로 저장 (productType을 CAP으로 설정)
        ...
        product.setProductType(ProductType.CAP);
    ...
    @GetMapping("/{id}")
    public Product capDetail(@PathVariable("id") Long id){
    ...
    public List<Product> findAll(){
        return productService.findAllActive().stream()
                .filter(p -> p.getProductType() == ProductType.CAP)
                .toList();
    }

    @GetMapping("/new")
    public List<Product> findNewCaps(){
